...
def preprocess_single(image, bkg_color=np.array([144, 72, 17])):
    img = np.mean(image[34:-16:2, ::2] - bgk_color, axis=-1) / 255.
    return img
...
def collect_trajectories(envs, policy, tmax=200, nrand=5):
    ...
    for t in range(tmax):
        ...
        # we take one action and skip game foward
...
def surrogate(policy, old_probs, states, actions, rewards,
    ...
    action = torch.tensor(actions, dtype=torch.int8, device=device)
...
def clipped_surrogate(policy, old_probs, states, actions, rewards,
    ...
    mean = np.mean(rewards_future, axis=1)
    std = np.std(rewards_future, axis=1) + 1.e-10
    ...
    # add in 1.e-10 to avoid log(0) which gives nan
    entropy = -(new_probs * torch.log(old_probs + 1.e-10)
                + (1. - new_probs) * torch.log(1. - old_probs + 1.e-10))
